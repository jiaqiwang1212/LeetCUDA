"""
Triton implementations of all ELU kernels from elu.cu.

Variants and their CUDA analogues:
  f32         - FP32, BLOCK_SIZE=256   (1 float/thread, matches elu_f32_kernel)
  f32x4       - FP32, BLOCK_SIZE=1024  (4 floats/thread via float4 in CUDA)
  f16         - FP16, BLOCK_SIZE=256   (1 half/thread, matches elu_f16_kernel)
...
ELU(x, alpha=1.0) = x if x > 0 else alpha * (exp(x) - 1)
"""
...
def elu_f32_kernel(
    ...
    y = tl.where(x > 0, x, tl.math.exp(x) - 1.0)
...
def elu_f16_kernel(
    ...
    x = tl.load(x_ptr + offsets, mask=mask)
    xf = x.to(tl.float32)
    y = tl.where(xf > 0, xf, tl.math.exp(xf) - 1.0).to(tl.float16)
    tl.store(y_ptr + offsets, y, mask=mask)


# Mirrors the half2 CUDA variant: each program processes 2 fp16 values.
@triton.jit
def elu_f16x2_kernel(
    x_ptr,
    y_ptr,
    N,
    BLOCK_SIZE: tl.constexpr,
...
# Mirrors elu_f16x8_kernel: 4 separate half2 sub-loads per program, each
# covering BLOCK_SIZE//4 elements.  The static_range loop is unrolled at
# compile time, generating 4 independent load/elu/store sequences just like
# the 4 explicit half2 loads in the CUDA source.
@triton.jit
def elu_f16x8_kernel(
    x_ptr,
    y_ptr,
    N,
    BLOCK_SIZE: tl.constexpr,
...
# Mirrors elu_f16x8_pack_kernel: a single contiguous 128-bit load
# (LDST128BITS in CUDA) for the full 8-element tile per program.
@triton.jit
def elu_f16x8_pack_kernel(
    x_ptr,
    y_ptr,
    N,
    BLOCK_SIZE: tl.constexpr,
...
def elu_f32(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 256
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    elu_f32_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)


def elu_f32x4(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 1024  # 4× the scalar variant
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    elu_f32x4_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)


def elu_f16(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 256
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    elu_f16_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)


def elu_f16x2(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 512  # 2× the scalar variant
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    elu_f16x2_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)


def elu_f16x8(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 2048  # 4 sub-loads of 512 elements each
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    elu_f16x8_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)


def elu_f16x8_pack(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 2048  # single 128-bit packed load per lane
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    elu_f16x8_pack_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)
...
def bench_elu(N, kernel):
    ...
    if kernel == "f32":
        fn = lambda: elu_f32(x, y)
    elif kernel == "f32x4":
        fn = lambda: elu_f32x4(x, y)
    elif kernel == "f32_torch":
        fn = lambda: torch.nn.functional.elu(x)
    elif kernel == "f16":
        fn = lambda: elu_f16(x, y)
    elif kernel == "f16x2":
        fn = lambda: elu_f16x2(x, y)
    elif kernel == "f16x8":
        fn = lambda: elu_f16x8(x, y)
    elif kernel == "f16x8_pack":
        fn = lambda: elu_f16x8_pack(x, y)
...
if __name__ == "__main__":
    ...
    for S, K in SKs:
        ...
        run_benchmark(elu_f32, x, "f32_triton", y)
        run_benchmark(elu_f32x4, x, "f32x4_triton", y)
        run_benchmark(torch.nn.functional.elu, x, "f32_torch")
        ...
        run_benchmark(elu_f16, x_f16, "f16_triton", y_f16)
        run_benchmark(elu_f16x2, x_f16, "f16x2_triton", y_f16)
        run_benchmark(elu_f16x8, x_f16, "f16x8_triton", y_f16)
        run_benchmark(elu_f16x8_pack, x_f16, "f16x8pack_triton", y_f16)
        run_benchmark(torch.nn.functional.elu, x_f16, "f16_torch")
        print("-" * 85)

    print("\nRunning triton.testing.perf_report benchmark (GB/s by N)...")
    bench_elu.run(print_data=True, show_plots=False, save_path="./")
