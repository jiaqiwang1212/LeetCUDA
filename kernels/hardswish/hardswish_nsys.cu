...
// Compile-time kernel selection: pass e.g. -DHARDSWISH_F32 to nvcc to profile only that variant.
// If none are defined, all variants are enabled.
#if !defined(HARDSWISH_F32) && !defined(HARDSWISH_F32X4) && !defined(HARDSWISH_F16) && \
    !defined(HARDSWISH_F16X2) && !defined(HARDSWISH_F16X8) && !defined(HARDSWISH_F16X8_PACK)
#define HARDSWISH_F32
#define HARDSWISH_F32X4
#define HARDSWISH_F16
#define HARDSWISH_F16X2
#define HARDSWISH_F16X8
#define HARDSWISH_F16X8_PACK
#endif

int main() {
  const int N = 1 << 20;

#if defined(HARDSWISH_F32) || defined(HARDSWISH_F32X4)
  // FP32 buffers
...
#if defined(HARDSWISH_F16) || defined(HARDSWISH_F16X2) || defined(HARDSWISH_F16X8) || defined(HARDSWISH_F16X8_PACK)
  // FP16 buffers
...
#ifdef HARDSWISH_F32
  // ------------------------------------------------------------------
  // hardswish_f32_kernel: block=256, grid=(N+255)/256
  {
    const int block = 256;
    const int grid = (N + 255) / 256;
    for (int i = 0; i < N_WARMUP; i++)
      hardswish_f32_kernel<<<grid, block>>>(d_x_f32, d_y_f32, N);
    cudaDeviceSynchronize();
    nvtxRangePush("hardswish_f32_kernel");
...
#ifdef HARDSWISH_F32X4
  // ------------------------------------------------------------------
  // hardswish_f32x4_kernel: block=64, grid=(N+255)/256
  {
    const int block = 64;
    const int grid = (N + 255) / 256;
    for (int i = 0; i < N_WARMUP; i++)
      hardswish_f32x4_kernel<<<grid, block>>>(d_x_f32, d_y_f32, N);
    cudaDeviceSynchronize();
    nvtxRangePush("hardswish_f32x4_kernel");
    for (int i = 0; i < N_ITER; i++)
      hardswish_f32x4_kernel<<<grid, block>>>(d_x_f32, d_y_f32, N);
    cudaDeviceSynchronize();
    nvtxRangePop();
...
#ifdef HARDSWISH_F16
  // ------------------------------------------------------------------
  // hardswish_f16_kernel: block=256, grid=(N+255)/256
  {
    const int block = 256;
    const int grid = (N + 255) / 256;
    for (int i = 0; i < N_WARMUP; i++)
      hardswish_f16_kernel<<<grid, block>>>(d_x_f16, d_y_f16, N);
    cudaDeviceSynchronize();
    nvtxRangePush("hardswish_f16_kernel");
    for (int i = 0; i < N_ITER; i++)
      hardswish_f16_kernel<<<grid, block>>>(d_x_f16, d_y_f16, N);
    cudaDeviceSynchronize();
    nvtxRangePop();
...
#ifdef HARDSWISH_F16X2
  // ------------------------------------------------------------------
  // hardswish_f16x2_kernel: block=128, grid=(N+255)/256
  {
    const int block = 128;
    const int grid = (N + 255) / 256;
    for (int i = 0; i < N_WARMUP; i++)
      hardswish_f16x2_kernel<<<grid, block>>>(d_x_f16, d_y_f16, N);
    cudaDeviceSynchronize();
    nvtxRangePush("hardswish_f16x2_kernel");
    for (int i = 0; i < N_ITER; i++)
      hardswish_f16x2_kernel<<<grid, block>>>(d_x_f16, d_y_f16, N);
    cudaDeviceSynchronize();
    nvtxRangePop();
...
#ifdef HARDSWISH_F16X8
  // ------------------------------------------------------------------
  // hardswish_f16x8_kernel: block=32, grid=(N+255)/256
  {
    const int block = 32;
    const int grid = (N + 255) / 256;
    for (int i = 0; i < N_WARMUP; i++)
      hardswish_f16x8_kernel<<<grid, block>>>(d_x_f16, d_y_f16, N);
    cudaDeviceSynchronize();
    nvtxRangePush("hardswish_f16x8_kernel");
    for (int i = 0; i < N_ITER; i++)
      hardswish_f16x8_kernel<<<grid, block>>>(d_x_f16, d_y_f16, N);
    cudaDeviceSynchronize();
    nvtxRangePop();
...
#ifdef HARDSWISH_F16X8_PACK
  // ------------------------------------------------------------------
  // hardswish_f16x8_pack_kernel: block=32, grid=(N+255)/256
  {
    const int block = 32;
    const int grid = (N + 255) / 256;
    for (int i = 0; i < N_WARMUP; i++)
      hardswish_f16x8_pack_kernel<<<grid, block>>>(d_x_f16, d_y_f16, N);
    cudaDeviceSynchronize();
    nvtxRangePush("hardswish_f16x8_pack_kernel");
    for (int i = 0; i < N_ITER; i++)
      hardswish_f16x8_pack_kernel<<<grid, block>>>(d_x_f16, d_y_f16, N);
    cudaDeviceSynchronize();
    nvtxRangePop();
