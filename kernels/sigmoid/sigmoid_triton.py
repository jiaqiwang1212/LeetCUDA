"""
Triton implementations of all Sigmoid kernels from sigmoid.cu.

Variants and their CUDA analogues:
  f32         - FP32, BLOCK_SIZE=256   (1 float/thread, matches sigmoid_f32_kernel)
  f32x4       - FP32, BLOCK_SIZE=1024  (4 floats/thread via float4 in CUDA)
  f16         - FP16, BLOCK_SIZE=256   (1 half/thread, matches sigmoid_f16_kernel)
...
# Mirrors sigmoid_f16x8_kernel: 4 separate half2 sub-loads per program, each
# covering BLOCK_SIZE//4 elements.  The static_range loop is unrolled at
# compile time, generating 4 independent load/sigmoid/store sequences just like
# the 4 explicit half2 loads in the CUDA source.
@triton.jit
def sigmoid_f16x8_kernel(
    x_ptr,
    y_ptr,
    N,
    BLOCK_SIZE: tl.constexpr,
...
# Mirrors sigmoid_f16x8_pack_kernel: a single contiguous 128-bit load
# (LDST128BITS in CUDA) for the full 8-element tile per program.
@triton.jit
def sigmoid_f16x8_pack_kernel(
    x_ptr,
    y_ptr,
    N,
    BLOCK_SIZE: tl.constexpr,
...
def sigmoid_f32(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 256
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    sigmoid_f32_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)


def sigmoid_f32x4(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 1024  # 4× the scalar variant
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    sigmoid_f32x4_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)


def sigmoid_f16(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 256
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    sigmoid_f16_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)


def sigmoid_f16x2(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 512  # 2× the scalar variant
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    sigmoid_f16x2_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)


def sigmoid_f16x8(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 2048  # 4 sub-loads of 512 elements each
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    sigmoid_f16x8_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)


def sigmoid_f16x8_pack(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 2048  # single 128-bit packed load per lane
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    sigmoid_f16x8_pack_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)
...
def check_correctness():
    ...
    ref_f32 = torch.sigmoid(x_f32)

    sigmoid_f32(x_f32, y_f32)
    assert torch.allclose(y_f32, ref_f32, atol=1e-5), "f32 mismatch"

    y_f32.zero_()
    sigmoid_f32x4(x_f32, y_f32)
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        ylabel="GB/s",
        plot_name="sigmoid-performance",
        args={},
    )
)
def bench_sigmoid(N, kernel):
    ...
    if kernel == "f32":
        fn = lambda: sigmoid_f32(x, y)
    elif kernel == "f32x4":
        fn = lambda: sigmoid_f32x4(x, y)
    elif kernel == "f32_torch":
        fn = lambda: torch.sigmoid(x)
    elif kernel == "f16":
        fn = lambda: sigmoid_f16(x, y)
    elif kernel == "f16x2":
        fn = lambda: sigmoid_f16x2(x, y)
    elif kernel == "f16x8":
        fn = lambda: sigmoid_f16x8(x, y)
    elif kernel == "f16x8_pack":
        fn = lambda: sigmoid_f16x8_pack(x, y)
...
if __name__ == "__main__":
    ...
    for S, K in SKs:
        ...
        run_benchmark(sigmoid_f32, x, "f32_triton", y)
        run_benchmark(sigmoid_f32x4, x, "f32x4_triton", y)
        run_benchmark(torch.sigmoid, x, "f32_torch")
        ...
        run_benchmark(sigmoid_f16, x_f16, "f16_triton", y_f16)
        run_benchmark(sigmoid_f16x2, x_f16, "f16x2_triton", y_f16)
        run_benchmark(sigmoid_f16x8, x_f16, "f16x8_triton", y_f16)
        run_benchmark(sigmoid_f16x8_pack, x_f16, "f16x8pack_triton", y_f16)
        run_benchmark(torch.sigmoid, x_f16, "f16_torch")
        print("-" * 85)

    print("\nRunning triton.testing.perf_report benchmark (GB/s by N)...")
    bench_sigmoid.run(print_data=True, show_plots=False, save_path="./")
