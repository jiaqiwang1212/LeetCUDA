"""
Triton implementations of all Swish (SiLU) kernels from swish.cu.

Variants and their CUDA analogues:
  f32         - FP32, BLOCK_SIZE=256   (1 float/thread, matches swish_f32_kernel)
  f32x4       - FP32, BLOCK_SIZE=1024  (4 floats/thread via float4 in CUDA)
  f16         - FP16, BLOCK_SIZE=256   (1 half/thread, matches swish_f16_kernel)
...
# Mirrors swish_f16x8_kernel: 4 separate half2 sub-loads per program, each
# covering BLOCK_SIZE//4 elements.  The static_range loop is unrolled at
# compile time, generating 4 independent load/swish/store sequences just like
# the 4 explicit half2 loads in the CUDA source.
@triton.jit
def swish_f16x8_kernel(
    x_ptr,
    y_ptr,
    N,
    BLOCK_SIZE: tl.constexpr,
...
# Mirrors swish_f16x8_pack_kernel: a single contiguous 128-bit load
# (LDST128BITS in CUDA) for the full 8-element tile per program.
@triton.jit
def swish_f16x8_pack_kernel(
    x_ptr,
    y_ptr,
    N,
    BLOCK_SIZE: tl.constexpr,
...
def swish_f32(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 256
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    swish_f32_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)


def swish_f32x4(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 1024  # 4× the scalar variant
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    swish_f32x4_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)


def swish_f16(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 256
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    swish_f16_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)


def swish_f16x2(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 512  # 2× the scalar variant
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    swish_f16x2_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)


def swish_f16x8(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 2048  # 4 sub-loads of 512 elements each
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    swish_f16x8_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)


def swish_f16x8_pack(x: torch.Tensor, y: torch.Tensor):
    N = x.numel()
    BLOCK_SIZE = 2048  # single 128-bit packed load per lane
    grid = (triton.cdiv(N, BLOCK_SIZE),)
    swish_f16x8_pack_kernel[grid](x, y, N, BLOCK_SIZE=BLOCK_SIZE)
...
def bench_swish(N, kernel):
    ...
    if kernel == "f32":
        fn = lambda: swish_f32(x, y)
    elif kernel == "f32x4":
        fn = lambda: swish_f32x4(x, y)
    elif kernel == "f32_torch":
        fn = lambda: x * torch.sigmoid(x)
    elif kernel == "f16":
        fn = lambda: swish_f16(x, y)
    elif kernel == "f16x2":
        fn = lambda: swish_f16x2(x, y)
    elif kernel == "f16x8":
        fn = lambda: swish_f16x8(x, y)
    elif kernel == "f16x8_pack":
        fn = lambda: swish_f16x8_pack(x, y)
...
if __name__ == "__main__":
    ...
    for S, K in SKs:
        ...
        run_benchmark(swish_f32, x, "f32_triton", y)
        run_benchmark(swish_f32x4, x, "f32x4_triton", y)
        run_benchmark(lambda t: t * torch.sigmoid(t), x, "f32_torch")
        ...
        run_benchmark(swish_f16, x_f16, "f16_triton", y_f16)
        run_benchmark(swish_f16x2, x_f16, "f16x2_triton", y_f16)
        run_benchmark(swish_f16x8, x_f16, "f16x8_triton", y_f16)
        run_benchmark(swish_f16x8_pack, x_f16, "f16x8pack_triton", y_f16)
        run_benchmark(lambda t: t * torch.sigmoid(t), x_f16, "f16_torch")
        print("-" * 85)

    print("\nRunning triton.testing.perf_report benchmark (GB/s by N)...")
    bench_swish.run(print_data=True, show_plots=False, save_path="./")
