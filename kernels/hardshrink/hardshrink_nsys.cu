...
// Compile-time kernel selection: pass e.g. -DHARDSHRINK_F32 to nvcc to profile only that variant.
// If none are defined, all variants are enabled.
#if !defined(HARDSHRINK_F32) && !defined(HARDSHRINK_F32X4) && !defined(HARDSHRINK_F16) && \
    !defined(HARDSHRINK_F16X2) && !defined(HARDSHRINK_F16X8) && !defined(HARDSHRINK_F16X8_PACK)
#define HARDSHRINK_F32
#define HARDSHRINK_F32X4
#define HARDSHRINK_F16
#define HARDSHRINK_F16X2
#define HARDSHRINK_F16X8
#define HARDSHRINK_F16X8_PACK
#endif

int main() {
  const int N = 1 << 20;

#if defined(HARDSHRINK_F32) || defined(HARDSHRINK_F32X4)
  // FP32 buffers
...
#if defined(HARDSHRINK_F16) || defined(HARDSHRINK_F16X2) || defined(HARDSHRINK_F16X8) || defined(HARDSHRINK_F16X8_PACK)
  // FP16 buffers
...
#ifdef HARDSHRINK_F32
  // ------------------------------------------------------------------
  // hardshrink_f32_kernel: block=256, grid=(N+255)/256
  {
    const int block = 256;
    const int grid = (N + 255) / 256;
    for (int i = 0; i < N_WARMUP; i++)
      hardshrink_f32_kernel<<<grid, block>>>(d_x_f32, d_y_f32, N);
    cudaDeviceSynchronize();
    nvtxRangePush("hardshrink_f32_kernel");
...
#ifdef HARDSHRINK_F32X4
  // ------------------------------------------------------------------
  // hardshrink_f32x4_kernel: block=64, grid=(N+255)/256
  {
    const int block = 64;
    const int grid = (N + 255) / 256;
    for (int i = 0; i < N_WARMUP; i++)
      hardshrink_f32x4_kernel<<<grid, block>>>(d_x_f32, d_y_f32, N);
    cudaDeviceSynchronize();
    nvtxRangePush("hardshrink_f32x4_kernel");
    for (int i = 0; i < N_ITER; i++)
      hardshrink_f32x4_kernel<<<grid, block>>>(d_x_f32, d_y_f32, N);
    cudaDeviceSynchronize();
    nvtxRangePop();
...
#ifdef HARDSHRINK_F16
  // ------------------------------------------------------------------
  // hardshrink_f16_kernel: block=256, grid=(N+255)/256
  {
    const int block = 256;
    const int grid = (N + 255) / 256;
    for (int i = 0; i < N_WARMUP; i++)
      hardshrink_f16_kernel<<<grid, block>>>(d_x_f16, d_y_f16, N);
    cudaDeviceSynchronize();
    nvtxRangePush("hardshrink_f16_kernel");
    for (int i = 0; i < N_ITER; i++)
      hardshrink_f16_kernel<<<grid, block>>>(d_x_f16, d_y_f16, N);
    cudaDeviceSynchronize();
    nvtxRangePop();
...
#ifdef HARDSHRINK_F16X2
  // ------------------------------------------------------------------
  // hardshrink_f16x2_kernel: block=128, grid=(N+255)/256
  {
    const int block = 128;
    const int grid = (N + 255) / 256;
    for (int i = 0; i < N_WARMUP; i++)
      hardshrink_f16x2_kernel<<<grid, block>>>(d_x_f16, d_y_f16, N);
    cudaDeviceSynchronize();
    nvtxRangePush("hardshrink_f16x2_kernel");
    for (int i = 0; i < N_ITER; i++)
      hardshrink_f16x2_kernel<<<grid, block>>>(d_x_f16, d_y_f16, N);
    cudaDeviceSynchronize();
    nvtxRangePop();
...
#ifdef HARDSHRINK_F16X8
  // ------------------------------------------------------------------
  // hardshrink_f16x8_kernel: block=32, grid=(N+255)/256
  {
    const int block = 32;
    const int grid = (N + 255) / 256;
    for (int i = 0; i < N_WARMUP; i++)
      hardshrink_f16x8_kernel<<<grid, block>>>(d_x_f16, d_y_f16, N);
    cudaDeviceSynchronize();
    nvtxRangePush("hardshrink_f16x8_kernel");
    for (int i = 0; i < N_ITER; i++)
      hardshrink_f16x8_kernel<<<grid, block>>>(d_x_f16, d_y_f16, N);
    cudaDeviceSynchronize();
    nvtxRangePop();
...
#ifdef HARDSHRINK_F16X8_PACK
  // ------------------------------------------------------------------
  // hardshrink_f16x8_pack_kernel: block=32, grid=(N+255)/256
  {
    const int block = 32;
    const int grid = (N + 255) / 256;
    for (int i = 0; i < N_WARMUP; i++)
      hardshrink_f16x8_pack_kernel<<<grid, block>>>(d_x_f16, d_y_f16, N);
    cudaDeviceSynchronize();
    nvtxRangePush("hardshrink_f16x8_pack_kernel");
    for (int i = 0; i < N_ITER; i++)
      hardshrink_f16x8_pack_kernel<<<grid, block>>>(d_x_f16, d_y_f16, N);
    cudaDeviceSynchronize();
    nvtxRangePop();
